...
export default function ReceivePanel() {
    ...
    while (iceCandidatesQueueRef.current.length > 0) {
      ...
      if (candidate) {
        try {
          ...
          console.log("[RECEIVER] Flushed Queued ICE Candidate");
        } catch (err) {
          console.error("[RECEIVER] Error adding ICE candidate from queue:", err);
        }
      }
    }
  };
  ...
  const handleOfferAndAnswer = useCallback(
    async (targetSenderId: string, offer: RTCSessionDescriptionInit) => {
      const pc = createPeerConnection(targetSenderId);
      try {
        console.log("[RECEIVER] Setting Remote Description (Offer)");
        await pc.setRemoteDescription(new RTCSessionDescription(offer));

        console.log("[RECEIVER] Creating and Setting Local Description (Answer)");
        ...
        // Process buffered ICE candidates AFTER answer is fully created and applied
